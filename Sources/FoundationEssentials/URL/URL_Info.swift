...
internal struct _URLFlags: OptionSet {
    ...
    static var isSpecialScheme:         Self { Self(rawValue: 1 << 24) }
}
#endif
...
extension _URLInfo {
    ...
    static func parse(
        ...
        case .posix: parsePOSIX(filePath, flags: &flags, isDirectory: isDirectory)
        case .windows: parseWindows(filePath, flags: &flags, isDirectory: isDirectory)
        }
    ...
    static func parse(
        ...
        var flags = _URLFlags.fileImplFlags
        let length = strlen(fileSystemRepresentation)
        guard length > 0 else {
            flags.insert(.hasDirectoryPath)
            let base = resolveBaseURL(base, updating: &flags)
            return (filePathInfo(string: "./", flags: flags), base)
...
extension _URLInfo {
    ...
    func replacing(
        ...
        let isDirectory = path.withUnsafeBufferPointer {
            hasDirectoryPath($0, pathEnd: $0.count, pathLength: $0.count)
        }
    ...
    func replacing(
        ...
        case .encoded:
            hasEncodedPath = true
        case .unknown:
            for i in newPath.indices {
                if newPath[i] == UInt8(ascii: "%") {
                    hasEncodedPath = true
                    break
                }
            }
            ...
            String(unsafeUninitializedCapacity: stringSpan.count) { buffer in
                ...
                if hasUser {
                    if flags.contains(.didEncodeUser) {
                        writeIndex += URLEncoder.percentDecodeUnchecked(
                            input: stringSpan.extracting(userRange),
                    ...
                    if hasPassword {
                        ...
                        if flags.contains(.didEncodePassword) {
                            writeIndex += URLEncoder.percentDecodeUnchecked(
                                input: stringSpan.extracting(passwordRange),
                                output: .init(rebasing: buffer[writeIndex...])
                            )!
                        } else {
                            writeIndex = buffer[writeIndex...].initialize(
                ...
                if hasHost {
                    if flags.contains(.didEncodeHost) {
                        if flags.contains(.hasSpecialScheme), _uidnaHook() != nil {
                            writeIndex = buffer[writeIndex...].initialize(
                                fromSpan: stringSpan.extracting(hostRange)
                            )
                        } else {
                            writeIndex += URLEncoder.percentDecodeUnchecked(
                                input: stringSpan.extracting(hostRange),
                                output: .init(rebasing: buffer[writeIndex...])
                            )!
                        }
                    } else {
                ...
                if flags.contains(.didEncodePath) {
                    writeIndex += URLEncoder.percentDecodeUnchecked(
                        input: stringSpan.extracting(pathRange),
                        output: .init(rebasing: buffer[writeIndex...])
                    )!
                } else {
                    writeIndex = buffer[writeIndex...].initialize(
                ...
                if hasQuery {
                    ...
                    if flags.contains(.didEncodeQuery) {
                        writeIndex += URLEncoder.percentDecodeUnchecked(
                            input: stringSpan.extracting(queryRange),
                            output: .init(rebasing: buffer[writeIndex...])
                        )!
                    } else {
                        writeIndex = buffer[writeIndex...].initialize(
                ...
                if hasFragment {
                    ...
                    if flags.contains(.didEncodeFragment) {
                        writeIndex += URLEncoder.percentDecodeUnchecked(
                            input: stringSpan.extracting(fragmentRange),
                            output: .init(rebasing: buffer[writeIndex...])
                        )!
                    } else {
                        writeIndex = buffer[writeIndex...].initialize(
